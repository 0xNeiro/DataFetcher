...
# TODO: Build an initial version of indexer to a GraphQL DB.
...
#TODO: To change for our private node / to be able to abuse of our setup
class EventScanner:
    ...
    For example, you can get all events from all contracts in the same scan.
    ...
    def get_suggested_scan_start_block(self):
        ...
        if end_block:
            return max(1, end_block - self.NUM_BLOCKS_RESCAN_FOR_FORKS) #1
        return 1 #1
    ...
    def scan(self, start_block, end_block, start_chunk_size=20, progress_callback=Optional[Callable]) -> Tuple[
        ...
        while current_block <= end_block:
            ...
            logger.debug(
                "Scanning events for blocks: %d - %d, chunk size %d, last chunk scan took %f, last logs found %d",
...
#Todo: Add a parallelization point
...
#TODO: Improve the current state to make it compatible with GraphQL DB
class JSONifiedState(EventScannerState):
        ...
        def __init__(self, name = "test-state.json"):
            self.state = None
            self.fname = name
        ...
        def process_event(self, block_when: datetime.datetime, event: AttributeDict) -> str:
            ...
            #TODO: Refacto this part to make it more modular with all events.
            ...
            #print(args) #uncomment if we want to see events
            print(args)
            ...
            self.state["blocks"][block_number] = data_reordered
